...
class Elite_Vaftire_Observer_ProductTireSizeBinderTest extends VF_TestCase
{
    ...
    {
		$product = $this->newProduct(self::ID);
        $this->setRequestParams( array('tire_type'=>VF_Tire_Catalog_TireProduct::SUMMER_ALL) );
        $this->bindTireSize( $product );
        
        $tireProduct = $this->newTireProduct(self::ID);
        $this->assertEquals( VF_Tire_Catalog_TireProduct::SUMMER_ALL, $tireProduct->tireType(), 'should be able to assign tire type' );
    }
